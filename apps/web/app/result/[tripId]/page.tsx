  ...
  return (
    ...
    >
      ...
      <p style={{ color: 'var(--muted)', marginBottom: '2.5rem', lineHeight: 1.6 }}>
        도시별 날씨와 분위기를 분석해 코디 이미지를 만드는 중입니다.
        보통 2–4분 소요됩니다.
      </p>
...
function GalleryView({ trip }: { trip: Trip }) {
  const completedJobs = trip.generation_jobs.filter(
...
export default function ResultPage({ params }: { params: { tripId: string } }) {
  ...
  const fetchTrip = useCallback(async () => {
    if (!WORKER_URL) {
      // Demo mode: show mock completed trip
      setTrip({
        id: tripId,
        status: 'completed',
