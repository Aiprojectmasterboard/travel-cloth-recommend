...
export function LoginModal() {
  const { showLoginModal, setShowLoginModal, loginWithGoogle, loginWithEmail, signUpWithEmail } = useAuth();
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center">
      <div className="absolute inset-0 bg-black/40 backdrop-blur-sm" onClick={() => { setShowLoginModal(false); resetForm(); setMode("login"); }} />

      <div className="relative w-[420px] max-w-[90vw] bg-white rounded-2xl p-8" style={{ boxShadow: "0 24px 48px rgba(0,0,0,.15)" }}>
        <button
          onClick={() => { setShowLoginModal(false); resetForm(); setMode("login"); }}
